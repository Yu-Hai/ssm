...
<script type="text/javascript">
	...
	/* 	var totalheight = 0; //定义一个总的高度变量
	 ...
	 function toSubmit(pageNo){
		 var chengyu=$("#chengyu").val();
		 if(typeof(pageNo) == "undefined"){
			 pageNo=1;
		 }
		 if($.trim(chengyu)!=""){
			 loadData(chengyu,"",pageNo);
		 }
	 }
	 

	function loadData(chengyu,start,pageNo,url) {
		var data = "chengyu="+chengyu+"&start="+start+"&pageNo="+pageNo;
		$
				.ajax({
					type : "POST",
					...
					success : function(msg) {
						...
						if (msg.responseCode == "0000") {
							$("#accordion").empty();
							if (msg.data != null) {
								$("#showInfo").text(msg.data.totalSize);
								...
								if (length > 0) {
									...
									if(chengyu!=""){
										var bean=new Page();
										bean.setPageNo(msg.data.pageNo);
										bean.setPageSize(5);
										bean.setTotalSize(msg.data.totalSize);
										bean.setPageDiv("pageDiv");
										bean.setFn("toSubmit");
										bean.doStartPage();
									}
